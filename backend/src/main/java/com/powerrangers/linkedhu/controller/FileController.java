...
import com.powerrangers.linkedhu.dto.ResponseFileDTO;
import com.powerrangers.linkedhu.dto.ResponseMessageDTO;
import com.powerrangers.linkedhu.entity.FileDB;
import com.powerrangers.linkedhu.service.FileStorageService;
...
public class FileController {
    ...
    public ResponseEntity<ResponseMessageDTO> uploadImage(@RequestParam("file") MultipartFile file) {
        String message = "";
        try {
            if(file.getContentType().equals("image/png")||file.getContentType().equals("image/jpeg")){

                storageService.storeAvatar(file);
                message = "Uploaded image succesfully: " + file.getOriginalFilename();
                return ResponseEntity.status(HttpStatus.OK).body(new ResponseMessageDTO(message));
            }
            message = "Wrong type: " + file.getOriginalFilename();
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseMessageDTO(message));
    ...
    public ResponseEntity<List<ResponseFileDTO>> getListFiles() {
        List<ResponseFileDTO> files = storageService.getAllFiles().map(dbFile -> {
